...
def evaluate(segmentation_module, loader, cfg, gpu_id, result_queue):
    ...
    for batch_data in loader:
        ...
        iou = intersectionAndUnion(pred, seg_label, cfg.DATASET.num_class)
        result_queue.put_nowait((acc, pix, iou))
...
def worker(cfg, gpu_id, start_idx, end_idx, result_queue):
    ...
    net_decoder.load_state_dict(
        torch.load(cfg.MODEL.weights_decoder, map_location=lambda storage, loc: storage), strict=False)
...
def main(cfg, gpus):
    ...
    # summary
    iou = iou_meter.average()
    # for i, _iou in enumerate(iou):
    #     print('class [{}], IoU: {:.4f}'.format(i, _iou))

    print('[Eval Summary]:')
    print('Mean IoU: {:.4f}, Accuracy: {:.2f}%'
          .format(iou, acc_meter.average()*100))
